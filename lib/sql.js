...
// COLLECTION.SQL
	...
	class SQL {
		...
		/// SQL.FUNCTIONS ///////////////////////////////////////////////////////////////////
			 ...
			 */
			static PLACEHOLD(key) { return (!!key ? 
			...
			static FORMAT  	(query, values, refs = {}) {
				...
				while (!!res.match(rgx) && trys<3) {
					res = res.replace(rgx, (txt, param, key, val) => {
						if (key.has(sl)) {
							key = key.split(sl).slice(1);
							key = key.length==2?key:key.concat([sl]);
							val = ref[key[0]].Requests.get(key[1]);
							val = `${val.Query.replace(/;+$/,'').trim()}${smi}`;
						} else {
							key = has(key); val = values[key]||''; 
						};
						return val;
					}); (res == old ? trys+1 : 0); old = res;
				}; 	res = res.replace(new RegExp(`(\\s*${smi})+`,'g'),';');
				LG.IF("\n%s\n%s\n\n", div, res); return res;
			}
			/**
			...
			static TYPE  	(field, next) {
				...
				switch (typ) {
					case 'BLOB': case 'LONG_BLOB': case 'VAR_STRING': val = field.string() || '';
