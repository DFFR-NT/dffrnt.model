	...
	class SQL {
		...
		/// SQL.FUNCTIONS ///////////////////////////////////////////////////////////////////
			...
			static PLACEHOLD(key) {  
				return (!!key ? new RegExp("([\\t ]*)\:("+key.toUpperCase()+")\:", "g") : 
					/([\t ]*)\:([A-Z]+|(?:[/][A-z]+)+)(?:\((GET|POST|PUT|DELETE|MIDDLEWARE)\))?\:/g
				); 
			}
			/**
			...
			static FORMAT  	(query, values, refs = {}) {
				...
				while (!!res.match(rgx) && trys<3) {
					res = res.replace(rgx, (txt, nbsp, key, method, val) => {
						if (key.has(sl)) {
							key = key.split(sl).slice(1);
							key = key.length==2?key:key.concat([sl]);
							// key = key.filter(k=>k!='/');
							val = ref[key[0]].Requests.get(key.last)[method];
				...
				LG.IF(ltm, div, res.replace(sgx,'\\\\\\\\$1'));
				return res.replace(sgx,'\\$1');
			}
			/**
			...
			static TYPE  	(field, next) {
				var nme = field.name, len = field.length, typ = field.type, val;
				switch (typ) {
			...
			static SOCKET  	(options) {
				...
				var cols, link, mtch, pnts, prms, qrys, sock, slsh, json, rslt,
					mchr = (txt,rgx)=>(txt.match(rgx)||['']),
					opts = Assign({}, optx, options||{}),
					cls  = 'SocketLink', CC = SQL.CONCAT, as = { as: 'item' };
				// -----------------------------------------------------------------------
				try {
					cols = opts.columns; link = opts.link;
					switch (ISS(link)) {
						case 'object': 	
							sock = Assign({}, defx, link); break;;
						default: if (!!link) {
							mtch =  link.match(regx.mch);
							if (!!!mtch) throw new SyntaxError(
								'SQL.SOCKET: Invalid Path Scheme'
							);
							...
							// };	
							pnts = 	mchr(link, regx.pnt.mch)
										.map(p=>p.replace(
											regx.pnt.mch, 
											regx.pnt.rep
										)	);
							prms =  mchr(link, regx.prm.mch)
										.map(p=>p.replace(regx.prm.mch,regx.prm.rep))
										.reduce((a,c)=>`${a}${c}`,'')
										.replace(regx.num.mch, regx.num.rep)
										.replace(encl.mch,encl.obj);
							qrys = 	mchr(link, regx.qry.mch)
										.map(p=>p.replace(regx.qry.mch,regx.qry.rep))
										.reduce((a,c)=>`${a}${c}`,'')
										.replace(regx.brk.mch, regx.brk.rep)
										.replace(regx.num.mch, regx.num.rep)
										.replace(regx.esc.mch, regx.esc.rep)
